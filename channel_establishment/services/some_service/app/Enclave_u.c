...
static sgx_status_t SGX_CDECL Enclave_ocall_sgx_init_quote(void* pms)
{
	...
	ms->ms_retval = ocall_sgx_init_quote(ms->ms_p_att_key_id, ms->ms_p_target_info);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_sgx_get_quote_size(void* pms)
{
	ms_ocall_sgx_get_quote_size_t* ms = SGX_CAST(ms_ocall_sgx_get_quote_size_t*, pms);
	ms->ms_retval = ocall_sgx_get_quote_size(ms->ms_p_att_key_id, ms->ms_p_quote_size);
...
static sgx_status_t SGX_CDECL Enclave_ocall_sgx_get_quote(void* pms)
{
	...
	ms->ms_retval = ocall_sgx_get_quote(ms->ms_p_report, ms->ms_p_att_key_id, ms->ms_p_qe_report_info, ms->ms_p_quote, ms->ms_quote_size);
...
static const struct {
	...
	97,
	{
		...
		(void*)Enclave_ocall_sgx_get_quote_size,
...
sgx_status_t run_server(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_run_server_t ms;
...
sgx_status_t run_client(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_run_client_t ms;
