...
typedef struct ms_run_server_t {
	sgx_status_t ms_retval;
} ms_run_server_t;

typedef struct ms_run_client_t {
	sgx_status_t ms_retval;
} ms_run_client_t;
...
typedef struct ms_ocall_sgx_init_quote_t {
	sgx_status_t ms_retval;
	sgx_att_key_id_t* ms_p_att_key_id;
	sgx_target_info_t* ms_p_target_info;
} ms_ocall_sgx_init_quote_t;

typedef struct ms_ocall_sgx_get_quote_size_t {
	sgx_status_t ms_retval;
	sgx_att_key_id_t* ms_p_att_key_id;
	uint32_t* ms_p_quote_size;
} ms_ocall_sgx_get_quote_size_t;

typedef struct ms_ocall_sgx_get_quote_t {
	sgx_status_t ms_retval;
	sgx_report_t* ms_p_report;
	sgx_att_key_id_t* ms_p_att_key_id;
	sgx_qe_report_info_t* ms_p_qe_report_info;
	uint8_t* ms_p_quote;
...
static sgx_status_t SGX_CDECL sgx_run_server(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_run_server_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_run_server_t* ms = SGX_CAST(ms_run_server_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = run_server();


	return status;
}

static sgx_status_t SGX_CDECL sgx_run_client(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_run_client_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_run_client_t* ms = SGX_CAST(ms_run_client_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = run_client();
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_run_server, 0, 0},
		{(void*)(uintptr_t)sgx_run_client, 0, 0},
...
sgx_status_t SGX_CDECL ocall_sgx_init_quote(sgx_status_t* retval, sgx_att_key_id_t* p_att_key_id, sgx_target_info_t* p_target_info)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_p_att_key_id = sizeof(sgx_att_key_id_t);
	...
	void *__tmp_p_att_key_id = NULL;
	void *__tmp_p_target_info = NULL;

	CHECK_ENCLAVE_POINTER(p_att_key_id, _len_p_att_key_id);
	CHECK_ENCLAVE_POINTER(p_target_info, _len_p_target_info);

	if (ADD_ASSIGN_OVERFLOW(ocalloc_size, (p_att_key_id != NULL) ? _len_p_att_key_id : 0))
		return SGX_ERROR_INVALID_PARAMETER;
	...
	if (p_att_key_id != NULL) {
		ms->ms_p_att_key_id = (sgx_att_key_id_t*)__tmp;
		__tmp_p_att_key_id = __tmp;
		memset(__tmp_p_att_key_id, 0, _len_p_att_key_id);
		__tmp = (void *)((size_t)__tmp + _len_p_att_key_id);
		ocalloc_size -= _len_p_att_key_id;
	} else {
		ms->ms_p_att_key_id = NULL;
	}
	...
	if (status == SGX_SUCCESS) {
		...
		if (p_att_key_id) {
			if (memcpy_s((void*)p_att_key_id, _len_p_att_key_id, __tmp_p_att_key_id, _len_p_att_key_id)) {
				sgx_ocfree();
				return SGX_ERROR_UNEXPECTED;
			}
		}
...
sgx_status_t SGX_CDECL ocall_sgx_get_quote_size(sgx_status_t* retval, sgx_att_key_id_t* p_att_key_id, uint32_t* p_quote_size)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_p_att_key_id = sizeof(sgx_att_key_id_t);
	size_t _len_p_quote_size = sizeof(uint32_t);

	ms_ocall_sgx_get_quote_size_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_sgx_get_quote_size_t);
	void *__tmp = NULL;
	...
	ms = (ms_ocall_sgx_get_quote_size_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_sgx_get_quote_size_t));
	ocalloc_size -= sizeof(ms_ocall_sgx_get_quote_size_t);

	if (p_att_key_id != NULL) {
		ms->ms_p_att_key_id = (sgx_att_key_id_t*)__tmp;
		if (memcpy_s(__tmp, ocalloc_size, p_att_key_id, _len_p_att_key_id)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
		__tmp = (void *)((size_t)__tmp + _len_p_att_key_id);
...
sgx_status_t SGX_CDECL ocall_sgx_get_quote(sgx_status_t* retval, sgx_report_t* p_report, sgx_att_key_id_t* p_att_key_id, sgx_qe_report_info_t* p_qe_report_info, uint8_t* p_quote, uint32_t quote_size)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_p_report = sizeof(sgx_report_t);
	size_t _len_p_att_key_id = sizeof(sgx_att_key_id_t);
	size_t _len_p_qe_report_info = sizeof(sgx_qe_report_info_t);
	...
	void *__tmp_p_qe_report_info = NULL;
	void *__tmp_p_quote = NULL;

	CHECK_ENCLAVE_POINTER(p_report, _len_p_report);
	CHECK_ENCLAVE_POINTER(p_att_key_id, _len_p_att_key_id);
	CHECK_ENCLAVE_POINTER(p_qe_report_info, _len_p_qe_report_info);
	...
	if (ADD_ASSIGN_OVERFLOW(ocalloc_size, (p_att_key_id != NULL) ? _len_p_att_key_id : 0))
		return SGX_ERROR_INVALID_PARAMETER;
	if (ADD_ASSIGN_OVERFLOW(ocalloc_size, (p_qe_report_info != NULL) ? _len_p_qe_report_info : 0))
		return SGX_ERROR_INVALID_PARAMETER;
	...
	if (p_qe_report_info != NULL) {
		ms->ms_p_qe_report_info = (sgx_qe_report_info_t*)__tmp;
		__tmp_p_qe_report_info = __tmp;
		if (memcpy_s(__tmp, ocalloc_size, p_qe_report_info, _len_p_qe_report_info)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
		__tmp = (void *)((size_t)__tmp + _len_p_qe_report_info);
		ocalloc_size -= _len_p_qe_report_info;
	} else {
		ms->ms_p_qe_report_info = NULL;
	}
	...
	if (status == SGX_SUCCESS) {
		if (retval) *retval = ms->ms_retval;
		if (p_qe_report_info) {
			if (memcpy_s((void*)p_qe_report_info, _len_p_qe_report_info, __tmp_p_qe_report_info, _len_p_qe_report_info)) {
				sgx_ocfree();
				return SGX_ERROR_UNEXPECTED;
			}
		}
